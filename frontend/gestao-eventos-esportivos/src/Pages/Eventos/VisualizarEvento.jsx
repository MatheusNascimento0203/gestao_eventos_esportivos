...
export default ({ open, setOpen, id }) => {
    ...
    const visualizarEvento = async (id) => {
        ...
        try {
            const response = await axios.get(`http://localhost:3000/findEvento/${id}`, {
                headers: {
                    Authorization: `Bearer ${token}`,
                },
            });
            const eventoData = response.data;
            eventoData.dataInicial = new Date(eventoData.dataInicial).toISOString().split("T")[0];
            eventoData.dataFinal = new Date(eventoData.dataFinal).toISOString().split("T")[0];
